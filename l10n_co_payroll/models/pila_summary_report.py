...
class PilaSummaryReport(models.Model):
    ...
    def _query(self, with_clause='', fields={}, groupby='', from_clause=''):
        ...
        select_ = """
            ...
            ep.name as empleado,
            --- NOVEDADES
            case when to_char(ct.contract_date_start,'yyyy-mm')=to_char(current_date - interval '1 month','yyyy-mm') then 'X' else '' end as ing,
            case when to_char(ct.contract_date_end,'yyyy-mm')=to_char(current_date - interval '1 month','yyyy-mm') then 'X' else '' end as ret,
            ...
            sum(case when ps.liquidar_por='vacaciones' then coalesce(ps.dias_vacaciones,0) else coalesce(ps.dias_a_pagar,0) end) as dias_pension,
            ...
            sum(psl."Aporte AFP"+psl."Aporte AFP Compañía") as aporte_pension,
            ---- SALUD
            ...
            sum(psl."Aporte EPS"+psl."Aporte EPS Compañía") as aporte_salud,
            ---- CCF
            ...
            sum(psl."Aporte CCF") as aporte_ccf,
            ---- RIESGOS
            rpmarl.codigo as codigo_riesgos,
            sum(case when ps.liquidar_por='vacaciones' then coalesce(ps.dias_vacaciones,0) else coalesce(ps.dias_a_pagar,0) end) as dias_riesgos,
            ...
            sum(psl."Aporte ARL") as aporte_riesgos,
            ---- PARAFISCALES
            sum(case when ps.liquidar_por='vacaciones' then coalesce(ps.dias_vacaciones,0) else coalesce(ps.dias_a_pagar,0) end) as dias_parafiscales,
            ...
            sum (psl."Aporte SENA"+psl."Aporte ICBF") as aporte_parafiscales,
            case when co.ley_1607='true' then 'Si' else 'No' end as exonerado_sena_icbf,
            ---- TOTAL
            sum(coalesce(psl."Aporte AFP",0)+coalesce(psl."Aporte AFP Compañía",0)+coalesce(psl."Aporte EPS",0)+coalesce(psl."Aporte EPS Compañía",0)+coalesce(psl."Aporte CCF",0)+coalesce(psl."Aporte ARL",0)+coalesce(psl."Aporte SENA",0)+coalesce(psl."Aporte ICBF",0)) as total_aportes
        """
        ...
        from_ = """
            ...
            left join (
                    select slip_id, sum(case when psl.code='AFP_TRA' then total end) as "Aporte AFP",
