...
class HrWorkEntry(models.Model):
    ...
    date_start = fields.Datetime(
        string="Start DateTime (compat)",
        compute="_compute_l10n_co_payroll_datetime_bounds",
        store=True,
    )
    date_stop = fields.Datetime(
        string="Stop DateTime (compat)",
        compute="_compute_l10n_co_payroll_datetime_bounds",
        store=True,
    )
    ...
    def _compute_l10n_co_payroll_datetime_bounds(self):
        for work_entry in self:
            if not work_entry.date:
                work_entry.date_start = False
                work_entry.date_stop = False
                continue

            date_value = fields.Date.to_date(work_entry.date)
            start_dt = datetime.combine(date_value, time(hour=5))
            duration_seconds = max(int(round((work_entry.duration or 0.0) * 3600)), 0)
            stop_dt = start_dt + timedelta(seconds=duration_seconds)
            if duration_seconds:
                stop_dt -= timedelta(seconds=1)

            work_entry.date_start = fields.Datetime.to_string(start_dt)
            work_entry.date_stop = fields.Datetime.to_string(stop_dt)
    ...
    def calcular_valor(self,tipo,horario,partes_dia,festivos,mapa_tipos_horas_extras,time_zone):
        for work_entry in self:
            ...
            if (date_stop_tz.hour != 23) or (date_stop_tz.minute != 59) or (date_stop_tz.hour == 23 and date_stop_tz.minute == 59 and date_stop_tz.second < 59):
            ...
            if tipo=="he":
                ...
                _logger.debug("Cantidad RN calculada para work_entry %s: %s", work_entry.id, work_entry.qty_rn)
