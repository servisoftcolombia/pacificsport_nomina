...
class PilasReport(models.Model):
    ...
    dias_vacaciones = fields.Integer(string='Dias Vacaciones', readonly=True)
    ...
    def _query(self, with_clause='', fields={}, groupby='', from_clause=''):
        ...
        select_ = """
            ...
            co.name as company,
            string_agg(pr.name,' - ') as lote,
            aac.name as cuenta_analitica,
            sum(dias_trabajados) as dias_a_pagar,
            case when sum(dias_trabajados)=0 then sum(dias_vacaciones) else 0 end as dias_vacaciones,
            ...
            case when rp.country_id = (select id from res_country where code = 'CO' limit 1) then 'NO' else 'SI' end extranjero,
            case when rp.country_id = (select id from res_country where code = 'CO' limit 1) then 'NO' else 'SI' end residente_exterior,
            'N/A' as radicacion_exterior,
            case when to_char(ct.contract_date_start,'yyyy-mm')=to_char(current_date - interval '1 month','yyyy-mm') then
                'Todos los sistemas (ARL, AFP, CCF, EPS)' else 'NO' end as ing,
            case when to_char(ct.contract_date_start,'yyyy-mm')=to_char(current_date - interval '1 month','yyyy-mm') then
                ct.contract_date_start end as fecha_ing,
            case when to_char(ct.contract_date_end,'yyyy-mm')=to_char(current_date - interval '1 month','yyyy-mm') then
                'Todos los sistemas (ARL, AFP, CCF, EPS)' else 'NO' end as ret,
            case when to_char(ct.contract_date_end,'yyyy-mm')=to_char(current_date - interval '1 month','yyyy-mm') then
                ct.contract_date_end end as fecha_ret,
            'N/A' as tde,  -- traslado desde otra EPS
            'N/A' as tae,  -- Traslado a otra EPS
            'N/A' as tdp,  -- Traslado desde otra AFP
            'N/A' as tap,  -- Traslado a otra AFP
            case when ps.tipo_variacion_salario = 'fijo' and psl."Variable">0 then 'X' else '' end as vsp,  -- Variación permanente de salario
            'N/A' as fecha_vsp,
            case when ps.tipo_variacion_salario = 'variable' and psl."Variable">0 then 'X' else '' end as vst,   --- Variación transitoria de trabajo
            case when pswd."LICENCIANR"=0 then round((ct.wage/30)::numeric*ps.nod_unpaid_leaves::numeric,2)::TEXT else 'NO' end as sln,    --- suspencion licencia no remunedara
            case
            ...
            case when pswd."INCCOMUN">0 then round(pswd."INCCOMUN",2)::text else 'NO' end as ige,   --- INCAPACIDAD GENERAL
            case
            ...
            case when pswd."LICMP">0 then round(pswd."LICMP",2)::text else 'NO' end as lma,   --- licencia de maternidad
            case
            ...
            ((psl."Vacaciones")::int/(case when (ps.dias_vacaciones)=0 then 1 else (ps.dias_vacaciones) end))*(case when vac.code is not null and to_char(vac.date_from,'dd')::int>1 then ps.dias_vacaciones else ps.dias_a_pagar end) as vac_lr,
            case
                when ps.liquidar_por<>'vacaciones' then null
            ...
            "Aporte FPV" as avp,   --- aportes voluntarios a pensiones
            'N/A' as vct,
            'N/A' as inicio_vct,
            'N/A' as fin_vct,
            case when pswd."INCLABORAL">0 then pswd."INCLABORAL"::text else 'NO' end as irl,  --- incapacidad riesgos laboraes
            ...
            sum(case when ps.liquidar_por='vacaciones' then coalesce(ps.dias_vacaciones,0) else coalesce(ps.dias_a_pagar,0) end) as dias_fp,
            sum(psl."BAS_SEG_SOC_AFP_EPS") as ibc_fp,
            round((psl."Aporte AFP"+psl."Aporte AFP Compañía")/case when (psl."BAS_SEG_SOC_AFP_EPS")=0 then 1 else (psl."BAS_SEG_SOC_AFP_EPS") end *100,2)||'%' as tarifa_fp,
            ...
            sum(case when ps.liquidar_por='vacaciones' then coalesce(ps.dias_vacaciones,0) else coalesce(ps.dias_a_pagar,0) end) as dias_salud,
            sum(psl."BAS_SEG_SOC_AFP_EPS") as ibc_salud,
            round((psl."Aporte EPS"+psl."Aporte EPS Compañía")/case when (psl."BAS_SEG_SOC_AFP_EPS")=0 then 1 else (psl."BAS_SEG_SOC_AFP_EPS") end * 100,2)||'%'  as tarifa_eps,
            sum(psl."Aporte EPS")  as valor_cotizacion_emp,
            sum(psl."Aporte EPS Compañía")  as valor_cotizacion_comp,
            sum(psl."Aporte EPS"+psl."Aporte EPS Compañía")  as valor_cotizacion_salud,
            ...
            sum(case when ps.liquidar_por='vacaciones' then coalesce(ps.dias_vacaciones,0) else coalesce(ps.dias_a_pagar,0) end) as dias_arl,
            ...
            sum(case when ps.liquidar_por='vacaciones' then coalesce(ps.dias_vacaciones,0) else coalesce(ps.dias_a_pagar,0) end) as dias_ccf,
        ...
        from_ = """
            hr_payslip ps
                ...
                inner join (
                    select slip_id,
                        sum(case when psl.code='EPS_TRA' then total end) as "Aporte EPS",
                        sum(case when psl.code='AFP_TRA' then total end) as "Aporte AFP",
                        sum(case when psl.code='EPS_COM' then total end) as "Aporte EPS Compañía",
                        sum(case when psl.code='AFP_COM' then total end) as "Aporte AFP Compañía",
                        sum(case when psl.code='ARL' then total end) as "Aporte ARL",
                        sum(case when psl.code='SENA' then total end) as "Aporte SENA",
                        sum(case when psl.code='ICBF' then total end) as "Aporte ICBF",
                        sum(case when psl.code='CCF' then total end) as "Aporte CCF",
                        sum(case when psl.code='AFC' then total end) as "Aporte AFC",
                        sum(case when psl.code='FPV' then total end) as "Aporte FPV",
                        sum(case when psl.code='FON_SOL_SOL' then total end) as "Aporte AFSP",
                        sum(case when psl.code='FON_SOL_SUB' then total end) as "Aporte AFSPS",
                        sum(case when psl.code='BAS_SEG_SOC_ARL' then total end) as "BAS_SEG_SOC_ARL",
                        sum(case when psl.code='BAS_SEG_SOC_AFP_EPS' then total end) as "BAS_SEG_SOC_AFP_EPS",
                        sum(case when psl.code='BAS_PRE_SOC' then total end) as "BAS_PRE_SOC",
                        sum(case when psl.code='BAS_PAR' then total end) as "BAS_PAR",
                        sum(case when psl.code='VAC' then total end) as "Vacaciones",
                        sum(case when rc.category_id=15 and psl.code not in ('SUELDO','VAC','LICMP','ANTICIPO') then total end) as "Variable"
                    from hr_payslip_line psl
                    inner join hr_salary_rule rc on psl.salary_rule_id=rc.id
                    group by slip_id) psl on (ps.id=psl.slip_id)
                left join hr_employee ep on (ps.employee_id=ep.id)
                left join hr_version ct on (ps.version_id=ct.id)
                left join account_analytic_account aac on (aac.id=ep.analytic_account)
                left join resource_calendar rc on (ct.resource_calendar_id=rc.id)   -----nuevo
                ...
                ) vac on (ep.id=vac.employee_id and (vac.date_from::date between ps.date_from::date and ps.date_to::date or vac.date_to::date between ps.date_from::date and ps.date_to::date))
                left join  (
                    Select le.employee_id, let.name, le.date_from::date, le.date_to::date, wet.code
                    from hr_leave le
                    inner join hr_leave_type let on (le.holiday_status_id=let.id)
                ...
                ) lnr on (ep.id=lnr.employee_id and (lnr.date_from between ps.date_from::date and ps.date_to::date or lnr.date_to between ps.date_from::date and ps.date_to::date))
                left join  (
                    Select le.employee_id, let.name, le.date_from::date, le.date_to::date, wet.code
                    from hr_leave le
                    inner join hr_leave_type let on (le.holiday_status_id=let.id)
                ...
                ) ige on (ep.id=ige.employee_id and (ige.date_from::date between ps.date_from::date and ps.date_to::date or ige.date_to::date between ps.date_from::date and ps.date_to::date))
                left join  (
                    Select le.employee_id, let.name, le.date_from::date, le.date_to::date, wet.code
                    from hr_leave le
                    inner join hr_leave_type let on (le.holiday_status_id=let.id)
                ...
                ) lmp on (ep.id=lmp.employee_id and (lmp.date_from::date between ps.date_from::date and ps.date_to::date or lmp.date_to::date between ps.date_from::date and ps.date_to::date))
                left join  (
                    Select le.employee_id, let.name, le.date_from::date, le.date_to::date, wet.code
                    from hr_leave le
                    inner join hr_leave_type let on (le.holiday_status_id=let.id)
                ...
                ) ilab on (ep.id=ilab.employee_id And (ilab.date_from between ps.date_from::date and ps.date_to::date or ilab.date_to between ps.date_from::date and ps.date_to::date))
            """
        ...
        where_ = """
            ...
            and ps.state in ('done', 'validated', 'paid') and ps.liquidar_por<>'cesantias' and ps.liquidar_por<>'prima' and ps.liquidar_por<>'intereses_cesantias' and ps.liquidar_por<>'definitiva'
        """
