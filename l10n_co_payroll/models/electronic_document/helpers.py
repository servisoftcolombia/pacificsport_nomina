...
def _require_module(module_name, package_name):
    try:
        return import_module(module_name)
    except ImportError as err:
        raise UserError(_("Falta la dependencia Python '%s' para el flujo DIAN de nomina electronica.") % package_name) from err


def _require_crypto():
    return _require_module("OpenSSL.crypto", "pyOpenSSL")


def _require_serialization():
    module = _require_module("cryptography.hazmat.primitives.serialization", "cryptography")
    return module


def _require_xmlsig():
    return _require_module("xmlsig", "xmlsig")


def _require_requests():
    return _require_module("requests", "requests")


def _load_pkcs12(crypto, key_file, passphrase=None):
    with warnings.catch_warnings():
        ...
        return crypto.load_pkcs12(base64.b64decode(key_file), passphrase)
...
class WsdlQueryHelper(object):
    ...
    def _prepare_header_and_sign(self):
        ...
        """
        xmlsig = _require_xmlsig()
        crypto = _require_crypto()
        serialization = _require_serialization()
        template = etree.fromstring(self._template_file)
        ...
        if sign is None:
            raise UserError(_("Ocurrió un error al crear la plantilla para firma digital."))
        ...
        created_time = datetime.datetime.now(tz=pytz.UTC)
        ...
        ctx.load_pkcs12(_load_pkcs12(crypto, self._key_file, self._passphrase))
        ...
        ctx.sign(sign)
        ctx.verify(sign)
        _logger.debug("Mensaje SOAP DIAN firmado correctamente.")
    ...
    def _send_request(self, xml_data):
        ...
        _logger.debug("Enviando solicitud SOAP DIAN a %s.", self._url)
        response = requests.post(self._url, headers=headers, data=data)
        _logger.debug("Codigo de respuesta del Webservice DIAN => %s", response.status_code)
        ...
        _logger.debug("Contenido de respuesta del Webservice DIAN:\n%s", content_string)
    ...
    def send_nomina_sync(self, zip_name, zip_data):
        ...
        body = etree.Element(etree.QName(nsd['soap'], 'Body'), nsmap={'soap': nsd['soap'], 'wcf': nsd['wcf']})
        send_nomina_sync_tag = etree.Element(etree.QName(nsd['wcf'], 'SendNominaSync'))
        ...
        soap.append(body)
        return self._send_request(xml_data=soap)
