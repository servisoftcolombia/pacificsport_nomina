...
class ElectronicDocument(models.Model):
    ...
    respuesta_soap = fields.Binary(string="Respuesta SOAP")
    respuesta = fields.Char("Mensaje de respuesta")
    ...
    @staticmethod
    def _xml_tag_text(xml_content, namespace, tag):
        qualified_tag = f'{{{namespace}}}{tag}'
        for item in xml_content.iter():
            if item.tag == qualified_tag and item.text:
                return item.text
        return False

    @staticmethod
    def _encode_response_soap(response):
        response_bytes = response if isinstance(response, bytes) else response.encode()
        return base64.b64encode(response_bytes)

    @staticmethod
    def _response_filename(consecutivo):
        return 'envio_{}_{}.xml'.format(
            consecutivo,
    ...
    def send_test_set_async(self, objeto, service, response_nsd):
        for documento in self:
            ...
            _logger.debug('Respuesta SendTestSetAsync recibida para documento %s', documento.id)
            if service.get_response_status_code() == 200:
                xml_content = etree.fromstring(response)
                track_id = self._xml_tag_text(xml_content, response_nsd['d'], 'ZipKey')

                document_key = False
                if objeto.company_id.ne_tipo_ambiente == '1':  # El método síncrono genera el CUNE como seguimiento
                    document_key = self._xml_tag_text(xml_content, response_nsd['c'], 'XmlDocumentKey')

                tracking_value = track_id or document_key
                if tracking_value:
                    documento.write({
                        'fecha_envio': datetime.now(),
                        'codigo_respuesta_envio': service.get_response_status_code(),
                        'track_id': tracking_value,
                        'estado': 'track_id'
                    })
    ...
    def get_status_zip(self, objeto, service, response_nsd, response):
        for documento in self:
            _logger.debug('Consultando GetStatusZip para documento %s', documento.id)
            if documento.model_id == "hr.payslip.electronic":
                if not response:
                    try:
                        response = service.get_status_zip(track_id=documento.track_id)
                        _logger.debug('Respuesta GetStatusZip recibida para documento %s', documento.id)
                    except Exception as e:
                ...
                xml_content = etree.fromstring(response)
                status_message = self._xml_tag_text(xml_content, response_nsd['b'], 'StatusMessage')
                status_description = self._xml_tag_text(xml_content, response_nsd['b'], 'StatusDescription')
                status_code = self._xml_tag_text(xml_content, response_nsd['b'], 'StatusCode')
                log_status = status_message or status_description or 'Error'
                if not status_message and not status_description:
                    text = self._xml_tag_text(xml_content, response_nsd['s'], 'Text')
                    if text:
                        raise ValidationError(text)
                    raise ValidationError('La respuesta de la DIAN no contiene estado de validación.')

                _logger.debug('Respuesta de validación DIAN para %s: %s', documento.id, log_status)

                estado = None
                if (
                    status_message == "Documento con errores en campos mandatorios."
                    or status_description == "Documento con errores en campos mandatorios."
                ):
                    estado = "procesado_con_errores_campos"
                else:
                    if status_description in ("Batch en proceso de validación.", "En proceso de validación.") or (
                        status_description and "Set de prueba con identificador" in status_description
                    ):
                        estado = "track_id"
                    else:
                        estado = "procesado_correctamente"
                        # Si al empleado se le activo la opción de enviar provisiones previas, se desmarca si el envio fue correcto
                        payslip_electronic_id = self.env['hr.payslip.electronic'].search(
                            [('electronic_document_id', '=', documento.id)])
                        if payslip_electronic_id and payslip_electronic_id.employee_id.send_previous_provisions:
                            payslip_electronic_id.employee_id.send_previous_provisions = False

                documento.write({
                    'fecha_envio': datetime.now(),
                    'codigo_respuesta_envio': status_code or service.get_response_status_code(),
                    'respuesta': status_description or log_status,
                    'nombre_archivo_respuesta_soap': self._response_filename(objeto.consecutivo),
                    'respuesta_soap': self._encode_response_soap(response),
                    "estado": estado
                })
            else:
                pass
    ...
    def enviar_documento_original_borrar(self):
        for documento in self:
            ...
            if documento.estado in (
                    "procesado_con_errores_firma") or not documento.peticion_xml_comprimida:  #No esta preparado o con errores de preparacion(firma errada)
            ...
            _logger.debug(
                'Código de respuesta DIAN para documento %s: %s',
                documento.id,
                service.get_response_status_code(),
            )
            if service.get_response_status_code() == 200:
                xml_content = etree.fromstring(response)
                ...
                if objeto.company_id.ne_tipo_ambiente in ('2'):
                    _logger.debug('Consultando estado de validación en pruebas para nómina %s', objeto.consecutivo)
                    documento.get_status_zip(objeto, service, response_nsd, False)
                elif objeto.company_id.ne_tipo_ambiente in ('1'):
                    _logger.debug('Procesando respuesta de ambiente productivo para nómina %s', objeto.consecutivo)
    ...
    def enviar_correo(self):
        pass
        # El envio de correo para documentos fiscales ajenos a nomina queda fuera de la fase 1.
