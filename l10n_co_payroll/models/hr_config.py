...
class HrConfig(models.TransientModel):
    ...
    not_work_entry_view_gantt = fields.Boolean(string='Habilitar creación de entradas de trabajo en View Gantt',
                                               default=False,
                                               config_parameter='payroll.not_work_entries_view_gantt',
                                               help='Habilita la creación de entradas de trabajo cuando se navega en '
                                                    'View Gantt de Entradas de trabajo')
    ...
    def get_account_salary_rule_template(self, account_data, account_code):
        ...
        account_id = self.env['account.account'].search(
            [("company_ids", "in", self.env.company.ids), ('code', '=', account_code)], limit=1)
        group = {}
                    ...
                    if len(account_code) > 2:
                        account_parent_id = self.env['account.account'].search(
                            [("company_ids", "in", self.env.company.ids),
                             ('code', '=', account_code[:len(account_code) - 2])], limit=1)
                        if account_parent_id:
                            cuenta['company_ids'] = self.env.company.ids
                            cuenta['reconcile'] = account_parent_id.reconcile
                            cuenta['account_type'] = account_parent_id.account_type
                            cuenta_grupo = self._get_account_group(account_parent_id.code).id
                            if not cuenta_grupo:
                                group['name'] = account_parent_id.name
                                group['code_prefix_start'] = account_parent_id.code
                                cuenta_grupo = self._create_account_group(group).id
                                group = {}
                            cuenta['group_id'] = cuenta_grupo
                            account_id = self.env['account.account'].create(cuenta)
                            groups = self._get_account_group(account_code)
                            if not groups.id:
                                group['name'] = cuenta['name']
                                group['code_prefix_start'] = account_code
                                group['parent_id'] = cuenta_grupo
                                self._create_account_group(group)
                        else:
                        ...
                        account_parent_id = self.env['account.account'].search(
                            [("company_ids", "in", self.env.company.ids),
                             ('code', '=', account_code[:len(account_code) - 1])], limit=1)
                        if account_parent_id:
                            cuenta['company_ids'] = self.env.company.ids
                            cuenta['reconcile'] = account_parent_id.reconcile
                            cuenta['account_type'] = account_parent_id.account_type
                        ...
                        cuenta['company_ids'] = self.env.company.ids
                        cuenta['reconcile'] = 0
                        cuenta['account_type'] = 'asset_current'
                        account_id = self.env['account.account'].create(cuenta)
                        groups = self._get_account_group(account_code)
                        if not groups.id:
                            group['name'] = cuenta['name']
    ...
    def get_account_salary_rule_template_2(self, account_data, account_code):
        ...
        for cuenta in account_data['account']:
            if cuenta['code'] == account_code:
                ...
                if len(account_code) > 2:
                    account_parent_id = self.env['account.account'].search(
                        [("company_ids", "in", self.env.company.ids), ('code', '=', account_code[:len(account_code) - 2])],
                        limit=1)
                    if account_parent_id:
                        cuenta['company_ids'] = self.env.company.ids
                        cuenta['reconcile'] = account_parent_id.reconcile
                    ...
                    account_parent_id = self.env['account.account'].search(
                        [("company_ids", "in", self.env.company.ids), ('code', '=', account_code[:len(account_code) - 1])],
                        limit=1)
                    if account_parent_id:
                        cuenta['company_ids'] = self.env.company.ids
                        cuenta['reconcile'] = account_parent_id.reconcile
                    ...
                    cuenta_busqueda = self.env['account.account'].search(
                        [('name', '=', cuenta['name']), ('company_ids', 'in', cuenta['company_ids']),
                         ('reconcile', '=', cuenta['reconcile']), ('account_type', '=', cuenta['account_type'])],
                        limit=1)
                    # print('hola', cuenta_busqueda.name)
                    if cuenta_busqueda.name is False:
                        account_id = self.env['account.account'].create(cuenta)
                    else:
    ...
    def _get_account_group(self, code_prefix_start):
        """
            Find account grup by code prefix.
            :param str code_prefix_start: code prefix to find.
        """
        domain = [('code_prefix_start', '<=', code_prefix_start), ('code_prefix_end', '>=', code_prefix_start)]
        company_group = self.env['account.group'].search(domain + [('company_id', '=', self.env.company.id)], limit=1)
        if company_group:
            return company_group
        return self.env['account.group'].search(domain + [('company_id', '=', False)], limit=1)

    def _get_latest_company_settings(self, company):
        return self.env['res.config.settings'].search(
            [('company_id', '=', company.id)],
            order='id desc',
            limit=1,
        )

    def _has_colombian_payroll_localization(self, company):
        latest_settings = self._get_latest_company_settings(company)
        chart_template = False
        for candidate in (
            getattr(self, 'chart_template', False),
            getattr(latest_settings, 'chart_template', False),
            getattr(company, 'chart_template', False),
        ):
            if candidate:
                chart_template = candidate
                break

        if chart_template == 'co':
            return True

        fiscal_country = company.account_fiscal_country_id or company.country_id
        if fiscal_country and fiscal_country.code == 'CO':
            return True

        return bool(
            company.country_id
            and company.country_id.code == 'CO'
            and self.env['ir.module.module'].search_count(
                [('name', '=', 'l10n_co'), ('state', '=', 'installed')]
            )
        )

    def _supported_model_values(self, model, values):
        return {field_name: value for field_name, value in values.items() if field_name in model._fields}

    def _create_account_group(self, values):
        group_values = dict(values)
        if 'company_id' in self.env['account.group']._fields and not group_values.get('company_id'):
            group_values['company_id'] = self.env.company.id
        return self.env['account.group'].create(group_values)

    def apply_template_payroll_colombia(self):
        for item in self:
            company = item.company_id or item.env.company
            if item._has_colombian_payroll_localization(company):
                ...
                if True:  # not self.aplicada:
                    ...
                    try:
                        ...
                        with open(route, 'r', encoding='utf-8') as file:
                            data = json.load(file)
                        # No need to close the file if you are using 'with' statement, it takes care of it.
                        ...
                        for datos in data['category']:
                            ...
                            if not exist:
                                ...
                                _logger.debug(
                                    "Creando categoria salarial para company_id=%s user_companies=%s",
                                    item.env.company.id,
                                    self.env.user.company_ids.ids,
                                )
                                self.env['hr.salary.rule.category'].create(
                                    item._supported_model_values(self.env['hr.salary.rule.category'], datos)
                                )
                    ...
                    try:
                        _logger.debug("Cargando hr_work_entry_type_data.json")
                        ...
                        for datos in data['entry']:
                            ...
                            if not existe:
                                self.env['hr.work.entry.type'].create(
                                    item._supported_model_values(self.env['hr.work.entry.type'], datos)
                                )
                    ...
                    try:
                        _logger.debug("Cargando hr_payroll_structure_type_data.json")
                        ...
                        for datos in data['type']:
                            existe = self.env['hr.payroll.structure.type'].search([('name', '=', datos['name'])],
                                                                                  limit=1)
                            _logger.debug(
                                "Validando tipo de estructura %s existente=%s",
                                datos['name'],
                                bool(existe),
                            )
                            if not existe:
                                ...
                                self.env['hr.payroll.structure.type'].create(
                                    item._supported_model_values(self.env['hr.payroll.structure.type'], datos)
                                )
                    ...
                    try:
                        _logger.debug("Cargando hr_payroll_structure_data.json")
                        ...
                        for datos in data['payroll']:
                            ...
                            _logger.debug(
                                "Validando estructura %s para company_id=%s existente=%s",
                                datos['name'],
                                self.env.company.id,
                                bool(existe),
                            )
                            if not existe:
                                ...
                                self.env['hr.payroll.structure'].create(
                                    item._supported_model_values(self.env['hr.payroll.structure'], datos)
                                )
                        file.close()
                    ...
                    try:
                        _logger.debug("Cargando hr_salary_rule_data.json")
                        ...
                        _logger.debug("Iniciando recreacion de reglas salariales")
                        ...
                        for datos in data['rule']:
                            ...
                            if datos['category_id']:
                                record_category_ids = self.env['hr.salary.rule.category'].search(
                                    [('code', '=', datos['category_id']), ('company_id', '=', item.env.company.id)],
                                    limit=1,
                                ).id
                            ...
                            self.env['hr.salary.rule'].create(
                                item._supported_model_values(self.env['hr.salary.rule'], datos)
                            )

                        _logger.debug("Finalizo recreacion de reglas salariales")
                    ...
                    # Insertar inputs
                    try:
                        _logger.debug("Cargando hr_payslip_input_type_data.json")
                        ...
                        with open(route, encoding='utf-8') as file:
                            data = json.load(file)
                        _logger.debug("Iniciando recreacion de entradas de nomina")
                        for datos in data['input']:
                            ...
                            if not exist_2:
                                ...
                                self.env['hr.payslip.input.type'].create(
                                    item._supported_model_values(self.env['hr.payslip.input.type'], datos)
                                )

                        _logger.debug("Finalizo recreacion de entradas de nomina")
                    ...
                    try:
                        ...
                        _logger.debug("Iniciando asociacion de cuentas a reglas salariales")
                        ...
                        record_id = self.env['hr.salary.rule'].search(
                            [('code', 'in', ['PRI_SER', 'CES', 'INT_CES', 'REL_PRI_SER']),
                        ...
                        record_id = self.env['hr.salary.rule'].search([('code', 'in',
                                                                        ['HED_MAN', 'HEN_MAN', 'HRN_MAN',
                                                                         'HEDDF_MAN', 'HENDF_MAN', 'HRDDF_MAN',
                                                                         'HRNDF_MAN', 'HED_AUT', 'HEN_AUT', 'HRN_AUT',
                                                                         'HEDDF_AUT', 'HENDF_AUT', 'HRDDF_AUT',
                                                                         'HRNDF_AUT']),
                        ...
                        account_debit_administracion = self.get_account_salary_rule_template(account_data, '510551')
                        account_debit_produccion = self.get_account_salary_rule_template(account_data, '720595')
                        account_debit_ventas = self.get_account_salary_rule_template(account_data, '520551')

                        record_id = self.env['hr.salary.rule'].search(
                            [('code', '=', 'DOTACION'), ('company_id', '=', self.env.company.id)])
                        ...
                        """
                        _logger.debug("SQL para insertar salary.rule.account faltantes: %s", sql)
                        self.env.cr.execute(sql)
                        results = results = self.env.cr.dictfetchall()
                        _logger.debug("Consulta de salary.rule.account retorno %s filas", len(results))
                        valores_insertar_salary_rule_account = []
                        ...
                        _logger.debug(
                            "Se prepararon %s relaciones salary.rule.account para insercion",
                            len(valores_insertar_salary_rule_account),
                        )
                        sql = """
                        ...
                        """
                        _logger.debug("Actualizando cuentas existentes de reglas archivadas")
                        ...
                        _logger.debug(
                            "Insertando %s relaciones salary.rule.account nuevas",
                            len(valores_insertar_salary_rule_account),
                        )
