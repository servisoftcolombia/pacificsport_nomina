...
class Contract(models.Model):
    ...
    def _onchange_date_start(self):
        # Si el contrato cuenta con asignación de vacaciones se actualiza la fecha de nextcall
        if self.employee_id:
            ...
            if allocation:
                last_month_birthday = self._last_month_birthday()
                if last_month_birthday == fields.Date.today():
                    allocation.nextcall = last_month_birthday
                else:
                    allocation.nextcall = last_month_birthday + relativedelta(months=+1)

    def _last_month_birthday(self):
    ...
    def write(self, vals):
        for rec in self:
            ...
            if rec.retencion_fuente == 'procedimiento2' and rec.withholding_percentage_id and not rec.withholding_percentage_id.contract_id:
                rec.withholding_percentage_id.contract_id = rec.id
            # Si se elimina el porcentaje fijo del contrato, también se elimina el registro en la tabla de porcentajes
            if 'withholding_percentage_id' in vals and vals['withholding_percentage_id'] is False and rec._origin.withholding_percentage_id.id:
                rec.env['historical.withholdings'].search([('id', '=', rec._origin.withholding_percentage_id.id)]).unlink()
            ...
            if ('state' in vals and vals['state'] == 'open' and (('tipo_salario' in vals and vals['tipo_salario'] in ('tradicional', 'integral')) or rec.tipo_salario in ('tradicional', 'integral'))) \
                    or (('tipo_salario' in vals and vals['tipo_salario'] in ('tradicional', 'integral')) and rec.state == 'open' and 'state' not in vals):
                rec.create_allocation_leave()

            return super(Contract, rec).write(vals)
    ...
    def create_allocation_leave(self):
        ...
        if len(leave_type) == 0:
            raise ValidationError('No se encuentra un tipo de ausencia para vacaciones.')
        elif len(leave_type) > 1:
            raise ValidationError('Existe mas de un tipo de ausencia para vacaciones.')
        ...
        if not allocation:
            ...
            vals_to_create = {
                ...
                'state': 'confirm',
                'number_of_days': 0,
                'date_from': datetime.combine(self.date_start, time(5, 0, 0))
            }
            ...
            if allocation.allocation_type != 'accrual':
                ...
                allocation.date_from = datetime.combine(self.date_start, time(5, 0, 0))
            # Validar que la asignación tenga nextcall
    ...
    def _get_contract_work_entries_values(self, date_start, date_stop):
        ...
        for record in self:
            ...
            # in master.
            resources_list = [record.env['resource.resource'], resource]
            ...
            tz_dates = {}
            for leave in record.env['resource.calendar.leaves'].search(leave_domain):
                for resource in resources_list:
                    if leave.resource_id.id not in [False, resource.id]:
                        continue
                    tz = tz if tz else pytz.timezone((resource or record).tz)
                    ...
                    dt0 = fields.Datetime.to_datetime(leave.date_from).astimezone(tz)
                    dt1 = fields.Datetime.to_datetime(leave.date_to).astimezone(tz)
            ...
            # Attendances
            default_work_entry_type_id = record._get_default_work_entry_type_id()
            default_work_entry_type = record.env['hr.work.entry.type'].search([('id', '=', default_work_entry_type_id)])
            ...
            for interval in real_leaves:
                ...
                if interval[0] == interval[1]:  # if start == stop
                    continue
                # leave_entry_type = record._get_interval_leave_work_entry_type(interval, leaves, None)
                leave_entry_type = record._get_interval_leave_work_entry_type(interval, leaves, [])
                ...
                contract_vals += [dict([
                    ...
                    ('state', 'draft'),
                    ('contract_id', record.id),
                ] + record._get_more_vals_leave_interval(interval, leaves))]
