...
class NewEntry(models.Model):
    ...
    contract_id = fields.Many2one("hr.version", string="Contrato salarial", ondelete="cascade")
    type_id = fields.Many2one("hr.payslip.input.type", string="Tipo de entrada", domain="[('appear_contract', '=', True)]")
    description = fields.Char(
        string="Descripcion",
        help="Para sindicato se debe indicar el porcentaje, para los demas la descripcion de la deduccion o devengado.",
    )
    value = fields.Float(string="Valor de la deduccion o devengado")
    biweekly = fields.Boolean(
        string="Quincenal",
        help="Se conserva por compatibilidad con registros historicos.",
    )
    period = fields.Float(string="Numero de periodos", help="Numero de periodos a descontar o aplicar.")
    ...
    def _compute_category(self):
        for record in self:
            category = self.env["hr.salary.rule"].search([("code", "=", record.type_id.code)], limit=1).category_id.name if record.type_id else False
            record.category = category or False
