...
class HolidaysAllocation(models.Model):
    ...
    work_entry_type_code = fields.Char(string='Código tipo de ausencia', related='holiday_status_id.work_entry_type_id.code', help='Permite identificar el código del tipo de ausencia (Usada en views)')
    ...
    def _onchange_holiday_status_id(self):
        ...
        if self.holiday_status_id.work_entry_type_id.code == 'VAC':
            ...
            if allocation:
                user_employee = self.env.context.get('default_employee_id') or self.env.user.employee_id
                # Omite validacion cuando el encargado de ausencias tiene asignacion y el tipo por defecto al crear es vacaciones
                if user_employee.id != self.employee_id.id:
            ...
            if self.contract_id:
                self.date_from = datetime.combine(self.sudo().contract_id.date_start, time(5, 0, 0))
        else:
    ...
    def _onchange_contract_id(self):
        if self.contract_id and self.holiday_status_id.work_entry_type_id.code == 'VAC':
            # Validación de asignación única por empleado y contrato
            self._onchange_holiday_status_id()

    @api.onchange('anticipated_vacations')
    def _onchange_anticipated_vacations(self):
        if self.employee_id:
            if self.employee_id.company_id.anticipated_vacation_limit < self.anticipated_vacations and self.employee_id.company_id.anticipated_vacation_limit != 0:
                raise UserError(f'Se supera el limite de días de vacaciones anticipadas, establecido por la compañía en {self.employee_id.company_id.anticipated_vacation_limit} dias.')
    ...
    def _sum_balance_allocation(self, vals_list):
        ...
        if not employee_id.contract_id:
            raise UserError(f"El empleado {employee_id.name}(id={employee_id.id}) no cuenta con un contrato.")
        ...
        # Si no tiene asignación, se notifica que es necesario que el empleado tenga un contrato o asignación activa
            ...
            raise UserError(f"Empleado {employee_id.name}(id={employee_id.id}) sin asignación de vacaciones activa.")
    ...
    def write(self, vals):
        ...
        if 'anticipated_vacations' in vals and not self.env.context.get('anticipated_vacations', False):
            # Permite modificar (aumentar o diminuir) dias anticipados y esto se refleje en number_of_days (solo cuando se realiza manualmente) 
            self.number_of_days += vals['anticipated_vacations'] - self.anticipated_vacations
        ...
        if len(vals_list) > 0:
            ...
            if 'saldo' in vals_list and vals_list['saldo'] is True:
                ...
                if allocation and vals_list['holiday_status_id'] == allocation.holiday_status_id.id and vals_list['contract_id'] == allocation.contract_id.id:
                    raise UserError(f"El empleado {employee_id.name}(id={employee_id.id}) solo puede tener una asignación de vacaciones por contrato")
                else:
    ...
    def cron_grouping_start_vacation_allocation(self, limit=500):
        ...
        if len(employee_ids) > 0:
            ...
            for employee_id in employee_ids:
                ...
                if not vacation_allocations:
                    if contract_active:
                        ...
                        values['date_from'] = datetime.combine(contract_active.date_start, time(5, 0, 0))
                ...
                if vacation_allocations:
                    ...
                    if vacation_active_contract and contract_active:
                        allocation_save = None
                        # Se toman todas las asignaciones de los empleados de tipo vacaciones y se agrupan en una
                        # sola para cada empleado
                        days_before_last_birthday_contract = 0
                        # _logger.info(f'\nvacation_active_contract {vacation_active_contract} \n contract_active: {
                        # contract_active} \nemployee_id: {employee_id}')
                        for pos, allocation in enumerate(vacation_active_contract):

                            # Verificar si la asignación se creo luego del ultimo cumpleaños del contrato, de ser asi
                            # debe ser dias anticipados
                            ...
                            if contract_active:
                                ...
                                # Se almacenan eso dias asignados luego de el ultimo cumpleaños del contrato para
                                # tenerlos como dias anticipados Estos dias al no ser cargados con plantilla (
                                # saldo=True) se pueden diferenciar por campo saldo
                            ...
                            if pos == 0:
                                ...
                                if allocation_save.name != 'Actualización de vacaciones versión 3.0':
                                    message = f"""<ul class="o_mail_thread_message_tracking">
                                                                <li>
                                                                    Descripción:
                                                                    <span> {allocation_save.name} </span> <span 
                                                                    class="fa fa-long-arrow-right" role="img" 
                                                                    aria-label="Cambiado" title="Cambiado"></span> 
                                                                    <span> Actualización de vacaciones versión 3.0 
                                                                    </span> </li> </ul>"""
                                    allocation_save.message_post(body=message)
                                    allocation_save.name = 'Actualización de vacaciones versión 3.0'
                ...
                for contract in contracts:
                    ...
                    # Filtrar ausencias y otras asignaciones según la fecha de inicio y fecha fin (si la tiene) del
                    # contrato.
                    if contract.state == 'open':
                        ...
                        leaves_of_contract = leaves.filtered(lambda leave: contract.date_start <= leave.create_date.date() <= contract.date_end)
            ...
            # print(f'\nallocations_employees: {allocations_employees}') La bolsa de asignaciones (única por
            # contrato) se configura para iniciar como asignación devengada (1.25 días/mes)
            for allocation_employee in allocations_employees:
                ...
                # Con base en la fecha de inicio del contrato calcula el ultimo cumpleaños del contrato y se obtiene
                # cuantos días de vacaciones debe recibir hasta el dia actual (dia de ejecución de la acción
                # planificada)
                if contract and not allocation_employee['other_contracts']:  # Solo se calcula esto a las asignaciones de contrato activo
                    ...
                    for leave in employee_leaves:
                        ...
                        # Para que en el calculo de 'number_of_days' no se tengan en cuenta la ausencia actual,
                        # se le cambia el leave_type temporalmente dado que en la funcion _leave_intervals_batch()
                        # busca este tipo de ausencia
                    ...
                    if total_allocation < 0:
                        # Si total_allocation es negativo, se asignan esos dias como anticipados
                        # El contexto permite asignar dias anticipados y que no los sume a los disponibles
                        allocation_employee['allocation_id'].with_context({'anticipated_vacations': True}).write({'anticipated_vacations': abs(total_allocation)})
                        total_allocation = 0
                    ...
                    allocation_employee['allocation_id'].date_from = datetime.combine(contract.date_start, time(5, 0, 0))
                    # Mensaje de odoo Boot en la asignación (Dias Asignados y cual concepto)
                    message = (f"Asignados {total_allocation} días por concepto de vacaciones devengadas restantes "
                               f"desde ultimo cumpleaños del contrato en la actualización versión 3.0")
    ...
    def _update_accrual(self):
        ...
        for holiday in holidays:
            ...
            start_date = holiday.employee_id.contract_id.date_start
            ...
            if employee_contract:
                ...
                if start_date > employee_contract.date_start:
                ...
                days_to_give = holiday.number_of_days
    ...
    def _default_holiday_status_id(self):
        '''
        Overwrite method to add company in domain
        '''
        if self.user_has_groups('hr_holidays.group_hr_holidays_user'):
            domain = [('valid', '=', True)]
        else:
            domain = [('valid', '=', True), ('allocation_type', '=', 'fixed_allocation')]
        domain.append(('company_id', 'in', self.env.context.get('allowed_company_ids', False)))
